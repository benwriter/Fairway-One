...
function courseHistoryFor(event){
  ...
  const holeHistory={};for(let h=1;h<=18;h++){const vals=[];rounds.forEach(x=>{const v=scoreFor(x.event,x.player.id,h);if(v!=null&&!isPickup(v))vals.push(Number(v))});if(vals.length)holeHistory[h]={last:vals[0],avg:avg(vals),best:Math.min(...vals),count:vals.length}}
...
function allPerformanceSummary(){
  ...
  const gross=rounds.map(x=>x.metrics.gross).filter(x=>x!=null),points=rounds.map(x=>x.metrics.points),puttRounds=rounds.filter(x=>x.metrics.holesTracked>0);
  return {rounds,bestGross:gross.length?Math.min(...gross):null,avgGross:gross.length?avg(gross):null,avgPoints:avg(points),bestPoints:Math.max(...points),avgPutts:puttRounds.length?avg(puttRounds.map(x=>x.metrics.putts)):null,totalPars:rounds.reduce((n,x)=>n+x.metrics.scoreCounts.pars,0),lastFive:rounds.slice(0,5)};
}
...
function achievementsFor(event,metrics){
  ...
  if(metrics.holes===18&&!metrics.pickups&&(!coursePrior.length||metrics.gross<Math.min(...coursePrior.map(x=>x.metrics.gross).filter(x=>x!=null))))out.push({icon:'★',title:'Course best',detail:coursePrior.length?`New low round at ${event.course.name}`:`First saved round at ${event.course.name}`});
...
function renderEventModal(){
  ...
  const holes=d.course.holes.map(h=>`<div class="hole-edit"><strong>Hole ${h.number}</strong><div class="tiny-grid labeled-hole-grid"><label><span>Par</span><input type="number" min="3" max="6" value="${h.par}" data-hole-par="${h.number}"></label><label><span>Index</span><input type="number" min="1" max="18" value="${h.si}" data-hole-si="${h.number}"></label><label><span>Metres</span><input type="number" min="0" max="1000" step="1" value="${h.distance??''}" placeholder="—" data-hole-distance="${h.number}"></label></div></div>`).join('');
