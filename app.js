...
function roundMetrics(event,player){
  ...
  const rows=(event.course?.holes||[]).map(h=>{const gross=scoreFor(event,player.id,h.number);if(gross==null)return null;return{hole:h.number,par:Number(h.par||0),gross:Number(gross),net:holeNet(event,player,h.number),pts:holeStableford(event,player,h.number)}}).filter(Boolean);
  ...
  return {holes:rows.length,gross,net,points,toPar:gross-par,frontGross:front.length?sum(front.map(x=>x.gross)):null,backGross:back.length?sum(back.map(x=>x.gross)):null,frontPoints:front.length?sum(front.map(x=>x.pts)):null,backPoints:back.length?sum(back.map(x=>x.pts)):null,bestStretch,scoreCounts,...stat};
}
...
function courseKey(event){return `${String(event?.course?.name||'').trim().toLowerCase()}|${String(event?.course?.tee||'').trim().toLowerCase()}`}
function courseHistoryFor(event){
  ...
  const gross=rounds.map(x=>x.metrics.gross),pts=rounds.map(x=>x.metrics.points);
  const holeHistory={};for(let h=1;h<=18;h++){const vals=[];rounds.forEach(x=>{const v=scoreFor(x.event,x.player.id,h);if(v!=null)vals.push(Number(v))});if(vals.length)holeHistory[h]={last:vals[0],avg:avg(vals),best:Math.min(...vals),count:vals.length}}
  return {rounds,count:rounds.length,last:rounds[0],bestGross:Math.min(...gross),avgGross:avg(gross),avgPoints:avg(pts),holeHistory};
}
function allPerformanceSummary(){
  const rounds=personalRounds();if(!rounds.length)return null;const avg=vals=>vals.reduce((a,b)=>a+b,0)/vals.length;
  const gross=rounds.map(x=>x.metrics.gross),points=rounds.map(x=>x.metrics.points),puttRounds=rounds.filter(x=>x.metrics.holesTracked>0);
  return {rounds,bestGross:Math.min(...gross),avgGross:avg(gross),avgPoints:avg(points),bestPoints:Math.max(...points),avgPutts:puttRounds.length?avg(puttRounds.map(x=>x.metrics.putts)):null,totalPars:rounds.reduce((n,x)=>n+x.metrics.scoreCounts.pars,0),lastFive:rounds.slice(0,5)};
}
...
function achievementsFor(event,metrics){
  ...
  if(metrics.holes===18&&(!coursePrior.length||metrics.gross<Math.min(...coursePrior.map(x=>x.metrics.gross))))out.push({icon:'★',title:'Course best',detail:coursePrior.length?`New low round at ${event.course.name}`:`First saved round at ${event.course.name}`});
  ...
  if(event.format==='stableford'){const priorPts=prior.filter(x=>x.event.format==='stableford').map(x=>x.metrics.points);if(!priorPts.length||metrics.points>Math.max(...priorPts))out.push({icon:'●',title:'Stableford best',detail:`${metrics.points} points`})}
...
function playerMomentum(event,player){
  const scored=(event.course?.holes||[]).map(h=>({h:h.number,s:scoreFor(event,player.id,h.number),p:h.par})).filter(x=>x.s!=null).slice(-3);if(!scored.length)return'—';const rel=scored.reduce((n,x)=>n+Number(x.s)-Number(x.p),0);return `${signed(rel)} last ${scored.length}`;
}
...
function renderHoleStrip(event){
  ...
  return `<div class="elite-hole-strip" aria-label="Jump to hole">${Array.from({length:18},(_,i)=>i+1).map(h=>{const hInfo=formatInfo(formatKeyForHole(event,h)),score=hInfo.entry==='team'&&team?teamScoreFor(event,team.id,h):(me?scoreFor(event,me.id,h):null),done=me?selfHoleConfirmed(event,me,h):(event.confirmedHoles||[]).includes(h);return `<button class="elite-hole-chip ${h===hole?'current':''} ${done?'done':''}" data-action="jump-hole" data-hole="${h}"><span>${h}</span><small>${score==null?'·':score}</small></button>`}).join('')}</div>`;
}
...
function renderCourseRecords(){
  const groups=new Map();personalRounds().forEach(x=>{const key=courseKey(x.event),row=groups.get(key)||{name:x.event.course?.name||'Course',tee:x.event.course?.tee||'',rounds:[],lastDate:x.event.date};row.rounds.push(x);if(String(x.event.date||'')>String(row.lastDate||''))row.lastDate=x.event.date;groups.set(key,row)});
  ...
  return `<section class="section course-records"><div class="section-head"><div><p class="eyebrow">COURSE HISTORY</p><h3>Your course records</h3></div></div><div class="course-record-list">${rows.map(r=>{const gross=r.rounds.map(x=>x.metrics.gross),pts=r.rounds.map(x=>x.metrics.points),avg=gross.reduce((a,b)=>a+b,0)/gross.length;return `<article><div><strong>${escapeHtml(r.name)}</strong><small>${escapeHtml(r.tee)} · ${r.rounds.length} round${r.rounds.length===1?'':'s'}</small></div><span><small>Best</small><b>${Math.min(...gross)}</b></span><span><small>Avg</small><b>${avg.toFixed(1)}</b></span><span><small>Pts</small><b>${(pts.reduce((a,b)=>a+b,0)/pts.length).toFixed(1)}</b></span></article>`}).join('')}</div></section>`;
}
...
async function shareRoundRecap(event){
  ...
  const cells=[['FRONT 9',m.frontGross??'—'],['BACK 9',m.backGross??'—'],['PARS',m.scoreCounts.pars],['BIRDIES+',m.scoreCounts.birdies+m.scoreCounts.eagles],['PUTTS',m.holesTracked?m.putts:'—'],['PENALTIES',m.holesTracked?m.penaltyStrokes:'—']];
...
function cardStatusMeta(card,complete=false){
  ...
  if(complete)return{key:'finished',label:'READY TO SUBMIT',detail:'18 holes confirmed'};
...
function personalCardMeta(event){
  ...
  const complete=shared&&team?(event.teamConfirmedHoles?.[team.id]||[]).length>=18:selfCardComplete(event,me);
  return cardStatusMeta(card,complete)
}
...
function renderIndividualBoard(event,key){
  const rows=(event.players||[]).filter(player=>event.eventMode!=='tournament'||(player.competitionStatus||'active')==='active').map(player=>{let stats,value='—';if(key==='stroke'){stats=strokeStats(event,player);value=signed(stats.toPar)}else{stats=stablefordStats(event,player);value=`${stats.points} pts`}return{player,stats,value,sort:key==='stroke'?stats.toPar:-stats.points}}).filter(x=>x.stats.holes>0||event.eventMode!=='tournament').sort((a,b)=>a.sort-b.sort);
...
function confirmCurrentHole(){
  ...
  if(info.entry==='team'){
    ...
    if(cloudTeamMode(evt,h)){
      ...
      saveState();queueCloudSync(evt,{immediate:true});if(done){toast('<strong>Your card is complete.</strong> Ready to finalize.');ui.tab='events';ui.eventView=evt.id;app.scrollTop=0;return render()}render();return toast(`Hole ${h} confirmed for ${mine.name}.`)
    }
    ...
    if(allDone&&evt.cloud?.role==='admin'){evt.status='complete';evt.completedAt=new Date().toISOString()}
    saveState();queueCloudSync(evt,{immediate:true});if(done){toast('<strong>Your round is complete.</strong> Ready to submit.');ui.tab='events';ui.eventView=evt.id;app.scrollTop=0;return render()}render();return toast(`Hole ${h} confirmed.`)
  }
...
function openScoreOverrideModal(event){
  ...
  const refresh=()=>{const p=players.find(x=>x.id===playerEl.value),h=Math.max(1,Math.min(18,Number(holeEl.value||1))),v=p?scoreFor(event,p.id,h):null;if(current)current.textContent=`Current score: ${v==null?'not entered':v} · Hole ${h}`;if(v!=null)grossEl.value=v};playerEl.addEventListener('change',refresh);holeEl.addEventListener('change',refresh);refresh();
...
function exportTournamentCsv(event){
  ...
  const rows=(event.players||[]).map(p=>{const g=playerGroup(event,p.id),stroke=strokeStats(event,p),stable=stablefordStats(event,p),card=cardForPlayer(event,p),competition=competitionStatusMeta(p.competitionStatus||'active');return [g?.name||'',g?.teeTime||event.teeTime,p.name,Number(p.hcp||0).toFixed(1),...Array.from({length:18},(_,i)=>scoreFor(event,p.id,i+1)??''),stroke.gross||'',stroke.net||'',stroke.holes?signed(stroke.toPar):'',stable.points||0,competition.label,card?.status||((playerProgress(event,p)>=18)?'finished':'playing')]});
...
function playerCardRows(event,player){
  return (event.course?.holes||[]).map(h=>{const gross=scoreFor(event,player.id,h.number),net=holeNet(event,player,h.number),pts=holeStableford(event,player,h.number);return `<div class="card-review-row"><span>${h.number}</span><span>Par ${h.par}</span><strong>${gross==null?'—':gross}</strong><span>${net==null?'—':`Net ${net}`}</span><span>${pts==null?'—':`${pts} pts`}</span></div>`}).join('')
}
function teamCardRows(event,team){
  return (event.course?.holes||[]).map(h=>{const gross=teamScoreFor(event,team.id,h.number);return `<div class="card-review-row team"><span>${h.number}</span><span>Par ${h.par}</span><strong>${gross==null?'—':gross}</strong><span>${gross==null?'—':signed(gross-h.par)}</span><span>Team</span></div>`}).join('')
}
function customPlayerCardRows(event,player){
  return (event.course?.holes||[]).map(h=>{const key=formatKeyForHole(event,h.number),info=formatInfo(key);if(info.entry==='team'&&player.teamId){const gross=teamScoreFor(event,player.teamId,h.number);return `<div class="card-review-row team"><span>${h.number}</span><span>Par ${h.par}</span><strong>${gross==null?'—':gross}</strong><span>${gross==null?'—':signed(gross-h.par)}</span><span>${escapeHtml(info.short)}</span></div>`}const gross=scoreFor(event,player.id,h.number),net=holeNet(event,player,h.number),pts=holeStableford(event,player,h.number);return `<div class="card-review-row"><span>${h.number}</span><span>Par ${h.par}</span><strong>${gross==null?'—':gross}</strong><span>${net==null?'—':`Net ${net}`}</span><span>${pts==null?escapeHtml(info.short):`${pts} pts`}</span></div>`}).join('')
}
function openSubmitCardModal(event){
  ...
  const complete=isTeam?(event.teamConfirmedHoles?.[team.id]||[]).length>=18:selfCardComplete(event,me);if(!complete)return toast('Confirm all 18 holes before submitting your card.');
